from google.cloud import language_v1
import six
import pandas as pd
from datetime import datetime
import numpy as np


def date_time_changer(date):
    date_datetime = datetime.strptime(
        str(date).replace("PDT", ""), "%a %b %d %H:%M:%S %Y"
    )
    return date_datetime.strftime("%Y-%m-%d")


def gcloud_analyze_sentiment(content):
    client = language_v1.LanguageServiceClient()

    if isinstance(content, six.binary_type):
        content = content.decode("utf-8")

    type_ = language_v1.Document.Type.HTML
    document = {"type_": type_, "content": content}

    encoding_type = language_v1.EncodingType.UTF8
    response = client.analyze_sentiment(
        request={
            "document": document,
            "encoding_type": encoding_type,
        }
    )
    return response


# Builds gcloud request content string by concatenating each row's text to the next one
def builds_sentiment_analysis_request_document(table):
    length_text_each_person = []
    text_everybody = ""

    for row in table.index:
        if (
            table["text"][row].strip(" ").endswith("?")
            or table["text"][row].strip(" ").endswith(".")
            or table["text"][row].strip(" ").endswith("!")
        ):
            text_everybody = "{}{} <br>".format(
                text_everybody,
                table["text"][row].strip(" "),
            )
            length_text_each_person.append(len(table["text"][row].strip(" ")))
        else:
            text_everybody = "{}{}.<br>".format(
                text_everybody,
                table["text"][row].strip(" "),
            )
            length_text_each_person.append(len(table["text"][row].strip(" ")))
    return length_text_each_person, text_everybody


# Converts sentiment analysis data to table ready data by putting the sentiment of every person into array
# If the text of a person is split into multiple sentences,
# it adds them up in the sentiment magnitude array and does the average of sentiment score
def converts_sentiment_analysis_data_to_table_ready_data(
    sentences_sentiment, length_text_each_person
):
    previous_offset = 0
    i = 0
    sentiment_score = []
    sentiment_magnitude = []
    for x in sentences_sentiment:
        if len(sentiment_magnitude) == 0:
            sentiment_magnitude.append(x.sentiment.magnitude)
            sentiment_score.append([x.sentiment.score])
        elif (
            x.text.begin_offset - previous_offset <= length_text_each_person[i]
        ):
            sentiment_magnitude[i] += x.sentiment.magnitude
            sentiment_score[i].append(x.sentiment.score)
        else:
            previous_offset = x.text.begin_offset
            sentiment_magnitude.append(round(x.sentiment.magnitude, 1))
            sentiment_magnitude[i] = round(sentiment_magnitude[i], 1)
            sentiment_score.append([round(x.sentiment.score, 1)])
            sentiment_score[i] = round(np.mean(sentiment_score[i]), 1)
            i += 1
    sentiment_score[i] = round(np.mean(sentiment_score[i]), 1)
    return sentiment_magnitude, sentiment_score


def gcloud_execution(number_of_randow_rows, data):
    # Reads random rows
    random_row_table = data.sample(number_of_randow_rows)

    (
        length_text_each_person,
        overall_text,
    ) = builds_sentiment_analysis_request_document(random_row_table)

    sentences_sentiment = gcloud_analyze_sentiment(overall_text).sentences

    (
        sentiment_magnitude,
        sentiment_score,
    ) = converts_sentiment_analysis_data_to_table_ready_data(
        sentences_sentiment, length_text_each_person
    )
    (
        random_row_table["sentiment"],
        random_row_table["magnitude"],
    ) = (
        sentiment_score,
        sentiment_magnitude,
    )
    random_row_table.insert(1, "method", "gcloud")

    return random_row_table
